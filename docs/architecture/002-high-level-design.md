...
* **Original source** https://github.com/DFE-Digital/sap-sector/blob/main/docs/architecture/high-level-design.md
* **Source author:** Hari Dupati
* **Updates from:** Manraj Singh Gill, Dan Murfitt
...
- compare schools using existing performance data
- add these schools to a "my schools" shopping cart-like cookie
...
| --------------------- | -------------------------------------------------------------------------------------------- |
| Web application       | Public-facing entry point. Renders search, detail and comparison journeys. Read-only. ([caveat](#14-gateway))          |
...
No Authentication is required ([caveat](#14-gateway)).
...
| Writes            | Data pipeline      | The application does no inserts, updates or deletes. ([caveat](#14-gateway))                         |
| Reads             | Web application    | Read-only queries issued through Dapper. ([caveat](#14-gateway))                                     |
...
Search is implemented using Full text search feature of PostgreSQL. The index is done part of Data ingestion scripts which produced by SapPub.Data executable. Decision to replace Lucense with Text search is recorded in [ADR](../adrs/020-fts-and-geolocation-in-postgres.md). The text search supports:

- School name text search

#### 5.3a Postcode searches

Additionally, postcodes are searched using the external Postcodes.io search service, due to LLPG/NLPG and Royal Mail license restrictions, and ease of integration.  

- Postcode-based search queries 

### 5.4 Identity and access data

The application doesn't use any Authentication or Authorisation as its available for public use. [caveat](#14-gateway)
...
SAP Public uses several external datasets covering schools, performance, destinations, attendance, inspection outcomes and related education metrics.
...
Other longer-term and or single use data sources are used, such as a list of Local Authorities and their websites, or breakfast clubs, these are updated infrequently and manually. 

A full list of data sources is available [Data Sources](../data/002-sources.md).
...
| Google Analytics, via Google Tag Manager | Client-side, from the user's browser | Standard GA page and interaction data | Loaded only when the `analytics_preference` cookie is set to `true`. | Container ID is environment-specific |
| Microsoft Clarity | Client-side, from the user's browser | Session recording and interaction heatmaps | Loaded only when the `analytics_preference` cookie is set to `true`. | This is behavioural recording on an authenticated service and should be reflected in the data protection position |
...
Right hand side of diagram is PostgreSQL, the authoritative database, queried read-only through materialised views. SAPPub.Infrstructure uses Dapper (ORM library) to perform queries. 
...
The context view shows SAP Public in its wider setting, including users, upstream data sources and external supporting services.
...
SAP Public sits in the middle. Upstream education datasets feed the platform through the data pipeline. Monitoring and analytics sit outside the core system boundary but are still operational dependencies.
...
The service reads from one place: PostgreSQL, through materialised views. That access is read-only ([caveat](#14-gateway)). Search is served using full-text search from PostgreSQL that is builds during data pipeline run from the same PostgreSQL data, so it is derived rather than a second source of truth.

---

## 9. Interactions and key flows (TODO)
...
- the application has no write permissions on the database, which limits what an application-level compromise could do ([caveat](#14-gateway))
...
* the static content directory is present
* the database can be queried

This is run frequently and reported back to StatusCake for alerts for the team. 

The `/healthcheck` endpoint report that the application will respond (i.e is ready) but no futher, this is run very frequently as a readiness check.
...
- the only thing the application writes is its own in-memory search index, which does not survive a restart ([caveat](#14-gateway))
...
- upstream dataset availability and quality, including DfE publication cycles, affect what the service can show and when
...
The gateway is explained in details here [Gateway documentation](../developers/006-Gateway.md), but is currently able to write user emails to allow for 7 days of access to the private beta environment, and then access will expire. 

This will be removed prior to go-live, and all data removed from the application.