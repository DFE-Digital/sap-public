...
SAP Public is a public-facing school information service. Anonymous users can search for schools, view establishment and performance information, and compare schools using curated and derived education datasets.

Upstream education datasets are ingested by a data pipeline, which cleans, normalises and loads them into PostgreSQL. The pipeline owns the database structure and is the only thing that writes to it. A web application hosted in Azure Kubernetes Service (AKS) reads from that database read-only, and uses a full-text search for search journeys. Users don't require to authenicate.

Note:- Only to support beta testing, Gateway page check is place which require user to registered with their email address to access the service.
...
**Search currently runs on a Full text-searach index.** See [Search data and direction of travel](./high-level-design.md#53-search-data-and-direction-of-travel) in the HLD.

**The service do not require authentication by default.** See [Users and user types](./high-level-design.md#4-users-and-user-types) in the HLD.
