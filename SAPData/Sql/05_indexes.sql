...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_establishment_ks5_destinations')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_establishment_ks5_destinations_id not created)', v_schema, 'v_establishment_ks5_destinations';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_establishment_ks5_destinations_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_establishment_ks5_destinations_id', v_schema, 'v_establishment_ks5_destinations';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_establishment_ks5_destinations_id', v_schema, 'v_establishment_ks5_destinations');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_establishment_ks5_destinations_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_establishment_ks2_attainment')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_establishment_ks2_attainment_id not created)', v_schema, 'v_establishment_ks2_attainment';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_establishment_ks2_attainment_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_establishment_ks2_attainment_id', v_schema, 'v_establishment_ks2_attainment';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_establishment_ks2_attainment_id', v_schema, 'v_establishment_ks2_attainment');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_establishment_ks2_attainment_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_establishment_ks5_performance')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_establishment_ks5_performance_id not created)', v_schema, 'v_establishment_ks5_performance';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_establishment_ks5_performance_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_establishment_ks5_performance_id', v_schema, 'v_establishment_ks5_performance';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_establishment_ks5_performance_id', v_schema, 'v_establishment_ks5_performance');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_establishment_ks5_performance_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_england_ks5_destinations')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_england_ks5_destinations_id not created)', v_schema, 'v_england_ks5_destinations';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_england_ks5_destinations_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_england_ks5_destinations_id', v_schema, 'v_england_ks5_destinations';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_england_ks5_destinations_id', v_schema, 'v_england_ks5_destinations');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_england_ks5_destinations_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_england_ks2_attainment')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_england_ks2_attainment_id not created)', v_schema, 'v_england_ks2_attainment';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_england_ks2_attainment_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_england_ks2_attainment_id', v_schema, 'v_england_ks2_attainment';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_england_ks2_attainment_id', v_schema, 'v_england_ks2_attainment');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_england_ks2_attainment_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_england_ks5_performance')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_england_ks5_performance_id not created)', v_schema, 'v_england_ks5_performance';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_england_ks5_performance_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_england_ks5_performance_id', v_schema, 'v_england_ks5_performance';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_england_ks5_performance_id', v_schema, 'v_england_ks5_performance');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_england_ks5_performance_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_la_absence')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_la_absence_id not created)', v_schema, 'v_la_absence';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_la_absence_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_la_absence_id', v_schema, 'v_la_absence';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_la_absence_id', v_schema, 'v_la_absence');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_la_absence_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_la_ks5_destinations')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_la_ks5_destinations_id not created)', v_schema, 'v_la_ks5_destinations';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_la_ks5_destinations_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_la_ks5_destinations_id', v_schema, 'v_la_ks5_destinations';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_la_ks5_destinations_id', v_schema, 'v_la_ks5_destinations');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_la_ks5_destinations_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_la_ks2_attainment')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_la_ks2_attainment_id not created)', v_schema, 'v_la_ks2_attainment';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_la_ks2_attainment_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_la_ks2_attainment_id', v_schema, 'v_la_ks2_attainment';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_la_ks2_attainment_id', v_schema, 'v_la_ks2_attainment');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_la_ks2_attainment_id';
    END IF;
  END IF;
END $$;
...
DECLARE
  v_schema text := current_schema();
BEGIN
  IF to_regclass(format('%I.%I', v_schema, 'v_la_ks5_performance')) IS NULL THEN
    RAISE NOTICE 'SKIP: view %.% does not exist (index idx_v_la_ks5_performance_id not created)', v_schema, 'v_la_ks5_performance';
  ELSE
    IF to_regclass(format('%I.%I', v_schema, 'idx_v_la_ks5_performance_id')) IS NULL THEN
      RAISE NOTICE 'CREATE: index %.% on %.% ("Id")', v_schema, 'idx_v_la_ks5_performance_id', v_schema, 'v_la_ks5_performance';
      EXECUTE format('CREATE INDEX %I ON %I.%I ("Id")', 'idx_v_la_ks5_performance_id', v_schema, 'v_la_ks5_performance');
    ELSE
      RAISE NOTICE 'OK: index %.% already exists', v_schema, 'idx_v_la_ks5_performance_id';
    END IF;
  END IF;
END $$;
